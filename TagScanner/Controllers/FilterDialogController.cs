namespace TagScanner.Controllers
{
    ...
    public class FilterDialogController : SdiController
    {
        ...
        private void Apply() => _libraryGridController.Filter = Predicate;
        ...
        private void BtnAddRootFilter_Click(object sender, EventArgs e) => AddFilter(TreeViewNodes);

        private void BtnAddRootGroup_Click(object sender, EventArgs e) => AddGroup(TreeViewNodes);

        private void BtnClearAll_Click(object sender, EventArgs e) => Clear();
        ...
        private void PopupAddChildFilter_Click(object sender, EventArgs e) => AddFilter(SelectedNode.Nodes);

        private void PopupAddChildGroup_Click(object sender, EventArgs e) => AddGroup(SelectedNode.Nodes);

        private void PopupInsertSiblingFilter_Click(object sender, EventArgs e) => InsertFilter(SelectedNode);

        private void PopupInsertSiblingGroup_Click(object sender, EventArgs e) => InsertGroup(SelectedNode);
        #endregion

        #region Control Events

        private void TreeView_AfterSelect(object sender, TreeViewEventArgs e) => UpdateControls();
        ...
        private TreeNode GetTarget(DragEventArgs e) => TreeView.GetNodeAt(TreeView.PointToClient(new Point(e.X, e.Y)));

        private void TreeView_ItemDrag(object sender, ItemDragEventArgs e) => TreeView.DoDragDrop(SelectedNode, DragDropEffects.Move);

        private void TreeView_MouseDown(object sender, MouseEventArgs e) => SelectedNode = TreeView.GetNodeAt(e.Location);

        #endregion

        #region Methods

        private void AddFilter(TreeNodeCollection nodes) => AddNode(nodes, _defaultFilterText);

        private void AddGroup(TreeNodeCollection nodes) => AddNode(nodes, _defaultGroupText);

        private void AddNode(TreeNodeCollection nodes, string text) => SelectedNode = nodes.Add(text);
        ...
        private void InsertFilter(TreeNode sibling) => InsertNode(sibling, _defaultFilterText);

        private void InsertGroup(TreeNode sibling) => InsertNode(sibling, _defaultGroupText);

        private void InsertNode(TreeNode node, string text) => SelectedNode = GetSiblings(node).Insert(node.Index, text);
        ...
        private bool IsGroupSelected() => IsQuantifier(SelectedNodeText);

        private static bool IsQuantifier(string text) => Metadata.QuantifierStrings.Contains(text);
        ...
        private bool Test(object track) => Function((ITrack)track);
        ...
        private void InvalidateFilter() => _function = null;

        protected override bool LoadFromStream(Stream stream, string format) => true;

        protected override bool SaveToStream(Stream stream, string format) => true;

        protected override void ClearDocument() => View.TreeView.Nodes.Clear();
