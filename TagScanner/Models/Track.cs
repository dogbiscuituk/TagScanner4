namespace TagScanner.Models
{
    ...
    public class Track : ITrack, INotifyPropertyChanged
    {
        ...
        public string Decade => Year > 0 ? string.Format("{0}0s", Year / 10) : string.Empty;
        ...
        public string DiscTrack => string.Format("{0} - {1}", DiscOf, TrackOf);
        ...
        public FileStatus FileStatus
        {
            get
            {
                ...
                {
                    case +1: return FileStatus.Pending;
                    case -1: return FileStatus.Updated;
                    default: return FileStatus.Current;
                };
            }
        }
        ...
        public string YearAlbum => string.Format("{0} - {1}", Year, Album);
...
#pragma warning restore 612, 618
            ...
            _year = (int)tag.Year;
            if (tag is TagLib.Image.ImageTag t)
            {
                ImageAltitude = t.Altitude ?? 0;
                ImageCreator = t.Creator;
                ImageDateTime = t.DateTime ?? DateTime.MinValue;
                ImageExposureTime = t.ExposureTime ?? 0;
                ImageFNumber = t.FNumber ?? 0;
                ImageFocalLength = t.FocalLength ?? 0;
                ImageFocalLengthIn35mmFilm = (int)(t.FocalLengthIn35mmFilm ?? 0);
                ImageISOSpeedRatings = (int)(t.ISOSpeedRatings ?? 0);
                ImageKeywords = t.Keywords;
                ImageLatitude = t.Latitude ?? 0;
                ImageLongitude = t.Longitude ?? 0;
                ImageMake = t.Make;
                ImageModel = t.Model;
                ImageOrientation = t.Orientation;
                ImageRating = (int)(t.Rating ?? 0);
                ImageSoftware = t.Software;
            }
        }
...
#pragma warning restore 612, 618
            ...
            if (tag is TagLib.Image.ImageTag t)
            {
                t.Altitude = ImageAltitude;
                t.Creator = ImageCreator;
                t.DateTime = ImageDateTime;
                t.ExposureTime = ImageExposureTime;
                t.FNumber = ImageFNumber;
                t.FocalLength = ImageFocalLength;
                t.FocalLengthIn35mmFilm = (uint)ImageFocalLengthIn35mmFilm;
                t.ISOSpeedRatings = (uint)ImageISOSpeedRatings;
                t.Keywords = ImageKeywords;
                t.Latitude = ImageLatitude;
                t.Longitude = ImageLongitude;
                t.Make = ImageMake;
                t.Model = ImageModel;
                t.Orientation = ImageOrientation;
                t.Rating = (uint)ImageRating;
                t.Software = ImageSoftware;
            }
        }
        ...
        private static bool SequenceEqual(IEnumerable<string> x, IEnumerable<string> y) => x == null ? y == null : y != null && Enumerable.SequenceEqual(x, y);
